...
(function ($) {
    ...
    $.ThumbnailGallery = function (el, options) {
		...
		var init = function() {
			...
			updateMainImage(ImageIds[0]);
			for (var i = 0; i < ImageIds.length; i++) {
				var button = $('<a href="#" class="gallery-button"></a>');
				button
					.css('display', 'block')
					.css('float', 'left')
                    .attr("data-imageId",ImageIds[i])
				var thumbImage = $('<img class="thumbnail-image"  src="' + ImageDisplayUrl + '?id=' + ImageList [i]+ '" />');
				thumbImage
					.css('display','block')
					.css('float','left');
		...
		function updateMainImage(imageId) {
		    currImageNumber = imageId;
			mainImage = $('<img src="' + ImageDisplayUrl + '?id=' + imageId+ '" id="main-image" />');
		...
		function updateSize() {
			if (thumbWidth && imageWidth) {
				...
				var numThumbs = ImageList.length;
